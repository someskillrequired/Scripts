...
from sprite_definitions import dict_zombies
...
map_items = {'7977696948211437512'  :'skelton?',
             '797064919307537569'   :'skulls?',
             '8239950961501591391'  :'rocks1?',
             '1251434074552384296'  :'rocks2?',
             '2714987650481479785'  :'rocks3?',
             '5768965425817495539'  :'?',
             '8625715695127884212'  :'?',
             '6179780658058987152'  :'Giant',
             '1040'                 :'Unit generator',
             '8971922455791567927'  :'dirtyground?',
             '2081813709740248161'  :'lamppost?',
             '16337623704957821120' :'blocked_path???',
             '4024093395365995584'  :'swamp terrain???',
             '293812117068830615'   :'vod???',
             '2584085031089263216'  :'vod???',
             '8702552346733362645'  :'vod???',
             '2584085031089263216'  :'ground item???',
             '14597207313853823957' :"OilSource",
             '1130949242559706282'  :"TruckA",
             '2767512748241551274'  :"something related to player? Nuetral item",
             '3195137037877540492'  :"team none? Nuetral item maybe map vision?",
...
class map:
    def __init__(self,directory,map_name,sevenzip_executable):
        ...
        self.svnzip_unzipped_file_path    = os.path.join(self.directory,"ZXGame_Data\\Levels\\custom_maps_unzipped_no_changes")
        self.unzipped_folder_path         = os.path.join(self.directory,f"ZXGame_Data\\Levels\\custom_maps_unzipped_no_changes\\{self.filename}")
        ...
        self.rezipped_folder_path         = os.path.join(self.directory,"custom_maps")
    ...
    def extract(self):
        ...
        try:
            ...
            command = [
                ...
                f'-o{Path(self.svnzip_unzipped_folder_path)}',  # Output directory
...
class YourGameClass:
    def __init__(self, current_map, directory, sevenzip_executable):
        ...
        sprite_base_path = r"ZXGame_Data\Sprites"
        self.sprite_directory = os.path.join(directory,sprite_base_path)
    ...
    def init_sprites(self):
        ...
        sprite_dict_zombies = dict_zombies(self.sprite_directory)
    ...
    def set_map(self):
        ...
        if selected_file_path:
            ...
            self.current_map = map(game_directory,selected_file_path,self.sevenzip_executable)
    ...
    def define_toolbar(self):
        def tab1():
            self.map_select_dropdown    = pygame_gui.elements.UIButton(relative_rect=pygame.Rect((self.screen.get_width()-self.ui_width, 60), (self.ui_width, 30)),text='Load Map',manager=self.uimanager)
            self.save_button            = pygame_gui.elements.UIButton(relative_rect=pygame.Rect((self.screen.get_width()-self.ui_width, 90), (self.ui_width, 30)),text='Save',manager=self.uimanager)
        ...
        self.tab1_button                = pygame_gui.elements.UIButton(relative_rect=pygame.Rect((self.screen.get_width()-(self.ui_width), 0), (self.ui_width/3, 30)),text='File',manager=self.uimanager)
        self.tab2_button                = pygame_gui.elements.UIButton(relative_rect=pygame.Rect((self.screen.get_width()-(self.ui_width/3) * 2, 0), (self.ui_width/3, 30)),text='Terrain',manager=self.uimanager)
        self.tab3_button                = pygame_gui.elements.UIButton(relative_rect=pygame.Rect((self.screen.get_width()-(self.ui_width/3), 0), (self.ui_width/3, 30)),text='Entities',manager=self.uimanager)
                
        if self.active_tab == "tab1":
            tab1()
        elif self.active_tab == "tab2":
            tab2()
        elif self.active_tab == "tab3":
            tab3()
        
        self.zoom_slider                = pygame_gui.elements.UIHorizontalSlider(relative_rect=pygame.Rect((self.screen.get_width()-self.ui_width, 320), (self.ui_width, 30)),start_value=2,value_range=(1, 10),manager=self.uimanager)
        
    def toolbar_button_handler(self,event):
        
        if event.ui_element == self.tab1_button:
            self.active_tab = "tab1"
            self.define_ui()
        if event.ui_element == self.tab2_button:
            self.active_tab = "tab2"
            self.define_ui()
        if event.ui_element == self.tab3_button:
            self.active_tab = "tab3"
            self.define_ui()
        ...
        if self.active_tab == "tab1":
            ...
            if event.ui_element == self.move_cc:
    ...
    def init_toolbar_vars(self):
        ...
        self.cell_size = int(5 * scale_factor)
    ...
    def handle_brush_selection(self,grid_x,grid_y):
        ...
        selected_option = self.layer_objects_dropdown.selected_option[0]
        #different versions of pygameui return either a tuple or a single calue
        if isinstance(brush_size,tuple):
            brush_size = brush_size[0]
    ...
    def handle_events(self, event, ui_button):
        ...
        if event.type == pygame.VIDEORESIZE:
            ...
            if event.ui_element == self.zoom_slider:
                self.zoom_level = event.value *50
                self.define_scroll_bars()
           
        elif event.type == pygame.MOUSEBUTTONDOWN and not ui_button:
    ...
    def build_image(self):
        ...
        while running:
            ...
            self.cell_size = int(5 * (self.zoom_level / 100.0))
            ...
            for event in events:
                if not self.handle_events(event,ui_button):
                    running = False
                    
            if self.mouse_button_held_down and not ui_button:
    ...
    def draw_command_center(self):
        for key, item in self.all_entities.items():
            ...
            elif "isRailway" not in self.all_entities[key]:
                if self.all_entities[key].get("IDTemplate type=System.UInt64, mscorlib","NA") not in map_items:
                    #(self.all_entities[key].get("IDTemplate type=System.UInt64, mscorlib"))
                    x = self.all_entities[key]["valuex"]
                    y = self.all_entities[key]["valuey"]
                    if self.all_entities[key]["Size"]:
                        temp = self.all_entities[key]["Size"]
                        tempfront, tempback = temp.split(";", maxsplit=1)
                        tempfront = int(1)  # Convert to int to use in range
                        tempback = int(1)    # Convert to int to use in range
                        # Draw a grid of squares based on tempfront and tempback
                        for i in range(tempfront):  # Iterate over tempfront
                            for j in range(tempback):  # Iterate over tempback
                                # Calculate new x and y for each square
                                ...
                                new_y = y + j - tempback/2
                                # Draw the square at the new position
                                pygame.draw.rect(self.screen, (0,0,0), pygame.Rect(new_x * self.cell_size- self.x_mapoffset, new_y * self.cell_size- self.y_mapoffset, self.cell_size, self.cell_size))

    def move_command_ceneter(self,y,x):
        self.move_cc_selected = False
        for key, item in self.all_entities.items():
            if "isCommandCenter" in self.all_entities[key]:
    ...
    def on_option_selection_change(self):
        if isinstance(self.layer_select_dropdown.selected_option,tuple):
            self.layer_select_option = self.layer_select_dropdown.selected_option[1]
        else:
...
if __name__ == '__main__':
    ...
    try:
        ...
        if len(lines) == 4:
            ...
            game_directory = lines[0].replace("/","\\").replace("\n","")
            r01 = game_directory.replace("/","\\").replace("\n","") + "\\ZXGame_Data\\Levels\\R01.dxlevel"
